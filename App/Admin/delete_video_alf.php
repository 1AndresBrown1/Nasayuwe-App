...
if (isset($_GET['video_id'])) {
    ...
    $deleteQuery = "DELETE FROM video_alf WHERE video_id = $videoId";
    $result = mysqli_query($conn, $deleteQuery);

    if ($result) {
        // Redirigir de vuelta a videos_alf_admin.php con un parámetro de éxito
        header("Location: videos_alf_admin?delete_status=success");
        exit();
    } else {
        ...
        header("Location: videos_alf_admin?delete_status=error");
        exit();
    }
} else {
    // Si no se proporciona el parámetro video_id, redirigir de vuelta
    header("Location: videos_alf_admin");
    exit();
}
?>
