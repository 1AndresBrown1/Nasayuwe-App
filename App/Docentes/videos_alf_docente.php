...
<div class="container p-4 mt-20 sm:mt-6">
    ...
    <form method="GET" action="videos_alf_docente" class="mb-5">
    ...
    <button class="btn me-2" onclick="document.getElementById('my_modal_5').showModal()">Agregar Video</button>
    <a href="uppdf_docente">
    ...
    <div class="container mx-auto p-4">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            if ($category_filter !== '') {
                $query = "SELECT * FROM `video_alf` WHERE `category` = '$category_filter' ORDER BY `video_id` ASC";
            } else {
                $query = "SELECT * FROM `video_alf` ORDER BY `video_id` ASC";
            }
            ...
            ?>
                    <div class="rounded-lg shadow-md overflow-hidden">
                        ...
                        <div class="p-4">
                            ...
                            <p class="text-sm text-gray-500"><strong>Categoría:</strong> <?php echo $row['category']; ?></p>
                    ...
                    <dialog id="edit_modal_<?php echo $row['video_id']; ?>" class="modal modal-bottom sm:modal-middle">
                        <div class="modal-box">
                            <form action="edit_video_alf_docentes" method="POST">
                                <div>
                                    <div>
                                        ...
                                        <div class="mt-4 mb-4">
                                            <p class="font-semibold mb-2">Categoría del Video</p>
                                            <input type="text" name="category" class="input input-bordered w-full max-w-xs" value="<?php echo $row['category']; ?>">
                                        </div>
                                    </div>
...
<dialog id="my_modal_5" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box">
        <form action="save_video_alf_docentes" method="POST">
            <div class="modal-content">
                ...
                <select name="category" class="select select-bordered w-full max-w-xs mt-2" required>
...
<dialog id="my_modal_6" class="modal modal-bottom sm:modal-middle">
    <div class="modal-box">
        <h3 class="font-bold text-lg">Estudiantes</h3>
        <div class="modal-action">
            <button class="btn">Cerrar</button>
        </div>
    </div>
...
<script>
    function deleteVideo(videoId) {
        if (confirm("¿Estás seguro de que deseas eliminar este video?")) {
            window.location.href = `delete_video_alf_docente?video_id=${videoId}`;
        }
    }
</script>
