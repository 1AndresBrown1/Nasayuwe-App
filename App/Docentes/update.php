...
if(isset( $_SESSION['docente'])){
    if(@$_GET['fdid'] &&  $_SESSION['docente']=='docente' ) {
        $id=@$_GET['fdid'];
        $result = mysqli_query($conexion ,"DELETE FROM feedback WHERE id='$id' ") or die('Error');
        header("location:evaluaciones?q=3");
    }
}

//delete estudiantes
if(isset( $_SESSION['docente'])){
    if(@$_GET['demail'] &&  $_SESSION['docente']=='docente' ) {
        $demail=@$_GET['demail'];
        ...
        header("location:evaluaciones?q=1");
    }
}

//remove quiz
if(isset( $_SESSION['docente'])){
    if(@$_GET['q']== 'rmquiz' &&  $_SESSION['docente']=='docente' ) {
        $eid=@$_GET['eid'];
        ...
        header("location:evaluaciones?q=5");
    }
}

//add quiz
if(isset( $_SESSION['docente'])){
    if(@$_GET['q']== 'addquiz' &&  $_SESSION['docente']=='docente') {
        $name = $_POST['name'];
        ...
        header("location:evaluaciones?q=4&step=2&eid=$id&n=$total");
    }
}

//add question
if(isset( $_SESSION['docente'])){
    if(@$_GET['q']== 'addqns' &&  $_SESSION['docente']=='docente'  ) {
        $n=@$_GET['n'];
        $eid=@$_GET['eid'];
        $ch=@$_GET['ch'];
        ...
        for($i=1;$i<=$n;$i++) {
            ...
            $c=$_POST[$i.'3'];
            $d=$_POST[$i.'4'];
            $qa=mysqli_query($conexion ,"INSERT INTO options VALUES  ('$qid','$a','$oaid')") or die('Error61');
            $qb=mysqli_query($conexion ,"INSERT INTO options VALUES  ('$qid','$b','$obid')") or die('Error62');
            $qc=mysqli_query($conexion ,"INSERT INTO options VALUES  ('$qid','$c','$ocid')") or die('Error63');
            $qd=mysqli_query($conexion ,"INSERT INTO options VALUES  ('$qid','$d','$odid')") or die('Error64');
            $e=$_POST['ans'.$i];
            switch($e) {
                case 'a':
                    $ansid=$oaid;
        ...
        header("location:evaluaciones?q=0");
    }
}
...
if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) {
    ...
    if($ans == $ansid) {
        ...
        $w++;
        // Corregir el puntaje para evitar que sea negativo
        $s = max(0, $s - $wrong);
    ...
    if($sn != $total) {
        $sn++;
        header("location:account?q=quiz&step=2&eid=$eid&n=$sn&t=$total")or die('Error152');
        ...
        if($rowcount == 0) {
            $q2=mysqli_query($conexion ,"INSERT INTO rank VALUES('$email','$s',NOW())")or die('Error165');
        } else {
            while($row=mysqli_fetch_array($q) ) {
                $sun=$row['score'];
            }
        ...
        header("location:account?q=result&eid=$eid");
    } else {
        header("location:account?q=result&eid=$eid");
    }
}
...
if(@$_GET['q']== 'quizre' && @$_GET['step']== 25 ) {
    ...
    $q=mysqli_query($conexion ,"UPDATE `rank` SET `score`=$sun ,time=NOW() WHERE email= '$email'")or die('Error174');
    header("location:account?q=quiz&step=2&eid=$eid&n=1&t=$t");
}
