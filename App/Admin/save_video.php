<?php
	...
	if(isset($_POST['save'])){
		...
		if (strpos($video_url, 'youtube.com') !== false) {
			...
			mysqli_query($conn, "INSERT INTO `video` (video_name, location, descripcion, category) VALUES ('$name', '$location', '$descripcion', '$category')") or die(mysqli_error($conn));
			header("Location: videos_docentes?status1=success1");
		} else {
			// Si no es un enlace de YouTube, mostrar un mensaje de error
			header("Location: videos_docentes?status1=error1");
		}
	}
?>
